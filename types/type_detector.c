...
int	detect_type(int fd)
{
	...
	if (memcmp(buffer, ELF_SIG, 4) == 0)
	{
		printf("Format:	\033[35mELF\033[0m\n");
		...
		memcpy(&identification_header, buffer, sizeof(elf_header_identification));
		if (identification_header.BITS_ARCH == 1) // 32 bits
		{
			...
			memcpy(&header, buffer, sizeof(elf_header_32));
			...
			for (int i=0; i<u16(header.PROGRAM_HEADER_ENTRY_COUNT); i++)
			{
				...
				printf("\t- OFFSET:		%x\n", u32(program_headers[i].OFFSET));
				printf("\t- PHYSICAL_ADDR:	%p\n", (void *)u32(program_headers[i].PHY_ADDR));
				printf("\n");
		...
		{
			printf("64 bit elf are not handled yet.\n");
			free(buffer);
			return 1;
		}
		else // unknown
		{
			printf("Unknown elf bit format.\n");
			free(buffer);
			return 1;
		}
	}
