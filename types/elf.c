...
{
    return (uint16_t)(uint8_t)b[0]
         | ((uint16_t)(uint8_t)b[1] << 8);
}

uint32_t u32(const char b[4])
{
    return (uint32_t)(uint8_t)b[0]
         | ((uint32_t)(uint8_t)b[1] << 8)
         | ((uint32_t)(uint8_t)b[2] << 16)
         | ((uint32_t)(uint8_t)b[3] << 24);
}

void	print_elf_file_report(elf_header_32 *header) // TODO: handle 64 bits
{
	printf("ELF file:\n");
	printf("\tBITS:			%s\n", (header->IDENTIFICATION.BITS_ARCH == 2)?"64":"32");
	printf("\tENDIANNESS:		%s\n", (header->IDENTIFICATION.ENDIANNESS == 2)?"MSB":"LSB");
	printf("\tHEADER VERSION:		%i\n", header->IDENTIFICATION.VERSION);
	printf("\tABI:			%s\n", get_abi_name(header->IDENTIFICATION.ABI));
	printf("\tABI VERSION:		%i\n", header->IDENTIFICATION.ABI_VERSION);

	printf("\tFILE TYPE:		%s\n", get_file_type(header->FILE_TYPE));
	printf("\tINSTRUCTION SET:	%s\n", get_instruction_set_name(header->TARGET_SYSTEM));
	printf("=================================================\n\n");
}
